...
export default async function Page({ params, searchParams }: Props) {
  ...
  if (id && subjectId) {
    try {
      const res = await getData<any>(`/students/teacher/${id}/education/${subjectId}/lessons`);
      if (res.success && res.data) {
        if (Array.isArray(res.data)) {
          lessonList = res.data;
        } else if (res.data.items && Array.isArray(res.data.items)) {
  ...
  // ===== 5. Fetch teacher data =====
  let teacherData = null;
  if (id && subjectId) {
    try {
      const res = await getData<any>(endpoints.StudentTeacherEducation.get(id, subjectId));
      if (res.success && res.data) {
