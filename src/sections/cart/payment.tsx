...
export default function Payment({ items: data, paymentList }: Props) {
  ...
  const handlePaySubmit = async () => {
    ...
    try {
      ...
      const response: any = await postData(endpoints.payment.post_all_payment, requestBody);

      // ✅ هنا نأخذ invoiceId من response.data
      if (response && response.data?.invoiceId) {
      ...
      <Box dir="rtl">
        <Grid container spacing={4} sx={{ maxWidth: 1200, mx: 'auto' }}>
          ...
          <Grid item xs={12} md={7}>
            ...
            <Typography fontWeight={600} sx={{ mt: 4, mb: 1 }}>
              كوبون خصم
            </Typography>
            <Box sx={{ position: 'relative' }}>
              <TextField
                ...
                {cartItemsList.map((item) => (
                  ...
                  >
                    ...
                    <Box sx={{ flex: 1 }}>
                      ...
                      <Typography variant="subtitle2" sx={{ color: '#00bcd4', mt: 0.5 }}>
                        {item.price} درهم
                      </Typography>
                    </Box>
                  </Box>
        ...
        {invoiceId && (
          <SuccessPurchaseDialog
            ...
            invoiceId={invoiceId} // ✅ الآن نرسل الـ ID بشكل صحيح
          />
        )}
      </Box>
    </Container>
