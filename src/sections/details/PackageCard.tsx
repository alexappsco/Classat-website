...
import SuccessPurchaseDialog from '../cart/successPurchaseDialog';
      ...
      <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth dir="rtl">
        ...
        <DialogActions sx={{ p: 3, pt: 0 }}>
          ...
          <Button
            onClick={handleBuyNow}
            variant="contained"
            sx={{ borderRadius: '50px', px: 3, bgcolor: primary.main, '&:hover': { bgcolor: primary.dark } }}
            disabled={isSubmitting || !selectedMethod}
          >
            {isSubmitting ? <CircularProgress size={24} color="inherit" /> : 'تأكيد الشراء'}
          </Button>
...
function PackageCard({
  ...
  return (
    <>
      ...
      >
        <Stack spacing={2} alignItems="center">
          <Box sx={{ width: 64, height: 64, borderRadius: '50%', bgcolor: '#E6F4FF', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <Box component="img" src="/favicon/flash.svg" alt="" sx={{ width: 32, height: 32 }} />
          </Box>
          <Typography variant="h6" fontWeight={700}>{title}</Typography>
          <Typography variant="body2" color="text.secondary">{descriptionLine1}<br />{descriptionLine2}</Typography>
          <Typography variant="body2" color="text.secondary">{hoursLabel}</Typography>
          <Typography variant="h5" sx={{ color: warning.main, fontWeight: 700, mt: 1 }}>{price} درهم</Typography>
          <Typography variant="body2" color="text.secondary">{discountText}</Typography>
        </Stack>
      ...
      <PackagePaymentModal
        open={isPaymentModalOpen}
        onClose={() => setIsPaymentModalOpen(false)}
        packageData={{ id, title, price, discountText, hoursLabel }}
        paymentList={paymentList}
        teacherId={teacher_id}
      />
    </>
