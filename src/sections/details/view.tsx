...
export default function InstructorProfileUI({
  ...
  const instructor = teacherData
    ? {
        ...
        image: teacherData.teacherImageUrl,
        role: `خبرة ${teacherData.yearsOfExperience} سنوات في ${teacherData.subjectName} (${teacherData.educationApproachName})`,
        skills: teacherData.subjectName,
        price: teacherData.teacherHourlyPrice,
        bio: teacherData.teacherBio,
        qualifications: [
          `خبرة ${teacherData.yearsOfExperience} سنوات`,
          `سعر الساعة: ${teacherData.teacherHourlyPrice} درهم`,
          `السعر بعد الضرائب والعمولة: ${teacherData.priceAfterTaxAndProfit} درهم`,
        ],
      }
    : {
        name: 'لا يوجد بيانات',
        ...
        skills: '-',
        price: 0,
        bio: 'لا توجد بيانات متاحة لهذا المدرس.',
        qualifications: [],
      };
      ...
      <Card sx={{ p: { xs: 2, md: 4 } }}>
        ...
        >
          ...
          <Stack spacing={1} flexGrow={1} textAlign={{ xs: 'center', md: 'left' }}>
            ...
            >
              <Icon icon="ic:round-star" color="#ffb400" width={22} />
              <Typography fontWeight={600}>{teacherData?.averageRating || 0}</Typography>
            </Stack>
          </Stack>
        </Stack>
        ...
        >
          <Tab label="عن المحاضر" value="about" icon={<Icon icon="mdi:account" width={20} />} iconPosition="start" sx={{ width: { xs: '50%', md: '17%' } }} />
          <Tab label="الدروس التعليمية" value="courses" sx={{ width: { xs: '65%', md: '17%' } }} icon={<img src="/favicon/Group.svg" alt="" />} />
          <Tab label="الباقات" value="packages" sx={{ width: { xs: '50%', md: '17%' } }} icon={<img src="/favicon/Vector.svg" alt="" />} />
          <Tab label="احجز جلسة خاصة" value="session" sx={{ width: { xs: '65%', md: '17%' } }} icon={<img src="/favicon/Mask group.svg" alt="" />} />
        </Tabs>

        {/* ---------- المحتوى ---------- */}
        <Box sx={{ mt: 4 }}>
          {tab === 'about' && <AboutSection teacherData={teacherData} />}
          {tab === 'courses' && (
            <EducationalLessons lessonList={lessonList} teacher_id={id} paymentList={paymentList} />
          )}
          {tab === 'packages' && (
            <PackagesSection packages={packagesData} teacher_id={id} paymentList={paymentList} />
          )}
          {tab === 'session' && (
            <LiveSectionTimeDetails
              title="جلسة خاصة"
              teacherId={id}
              educationApproachTypeStageGradeSubjectId={subjectId}
