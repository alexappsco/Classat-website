...
function PipBTN({ isMobile, isTab }: PipBTNProps) {
  ...
  const togglePipMode = async () => {
    ...
    if ('pictureInPictureEnabled' in document) {
      ...
      // Ensure a non-null 2D context for the draw loop.
      ...
      function drawCanvas() {
        ...
        try {
          // Keep canvas sized to PiP window.
...
const MicBTN = () => {
  ...
  const [micMenuAnchor, setMicMenuAnchor] = useState<null | HTMLElement>(null);
  const [speakerMenuAnchor, setSpeakerMenuAnchor] = useState<null | HTMLElement>(null);

  useMediaDevice({ onDeviceChanged });

  function onDeviceChanged(devicesPromise: Promise<any[]>) {
    void devicesPromise.then((devices) => {
      getMics();
      const newSpeakerList = devices.filter((device) => device.kind === 'audiooutput');
  ...
  const getMics = async () => {
    ...
    speakers?.length && setSpeakers(speakers);
  };
  ...
  const closeTooltip = () => setTooltipShow(false);
  ...
  const handleMicClose = () => setMicMenuAnchor(null);

  const handleSpeakerClick = (event: React.MouseEvent<HTMLElement>) => {
    setSpeakerMenuAnchor(event.currentTarget);
  };

  const handleSpeakerClose = () => setSpeakerMenuAnchor(null);
    ...
    <Box sx={{ display: 'inline-flex', alignItems: 'center' }}>
      <OutlinedButton
        ...
        renderRightComponent={() => (
          <>
            ...
            <Menu anchorEl={micMenuAnchor} open={Boolean(micMenuAnchor)} onClose={handleMicClose}>
              <Paper sx={{ bgcolor: '#1A1C23', minWidth: 200, p: 1 }}>
                <Typography sx={{ p: 1, color: 'white', fontWeight: 'bold' }}>
                  MICROPHONE
                </Typography>
                {mics.map(({ deviceId, label }, index) => (
                  <MenuItem
                    key={`mics_${deviceId}`}
                    onClick={() => handleMicSelect(deviceId, label)}
                ...
                <Typography sx={{ p: 1, color: 'white', fontWeight: 'bold' }}>SPEAKER</Typography>
                {speakers.map(({ deviceId, label }, index) => (
                  <MenuItem
                    key={`speakers_${deviceId}`}
                    onClick={() => handleSpeakerSelect(deviceId, label)}
      ...
      >
        <Paper sx={{ borderRadius: 1, bgcolor: 'black', p: 0.75 }}>
          <Typography sx={{ color: 'white', fontSize: '1rem' }}>Change microphone</Typography>
        </Paper>
      </Box>
    </Box>
  );
    ...
    <Box sx={{ display: 'inline-flex', alignItems: 'center' }}>
      <OutlinedButton
        ...
        onClick={async () => {
          ...
          if (!localWebcamOn) {
            const created = await getVideoTrack({ webcamId: selectedWebcam.id ?? undefined });
            stream = created ?? undefined;
          }
        ...
        renderRightComponent={() => (
          <>
            ...
            <Menu anchorEl={anchorEl} open={Boolean(anchorEl)} onClose={handleClose}>
              <Paper sx={{ bgcolor: '#1A1C23', minWidth: 200, p: 1 }}>
                <Typography sx={{ p: 1, color: 'white', fontWeight: 'bold' }}>WEBCAM</Typography>
                {webcams.map(({ deviceId, label }, index) => (
                  <MenuItem
                    key={`webcams_${deviceId}`}
                    onClick={() => handleWebcamSelect(deviceId, label)}
      ...
      >
        <Paper sx={{ borderRadius: 1, bgcolor: 'black', p: 0.75 }}>
          <Typography sx={{ color: 'white', fontSize: '1rem' }}>Change webcam</Typography>
        </Paper>
      </Box>
    </Box>
  );
...
export function BottomBar({ bottomBarHeight, setIsMeetingLeft }: BottomBarProps) {
  ...
  const RecordingBTN = () => {
    ...
    const getTooltip = () => {
      ...
      if (recordingState === Constants.recordingEvents.RECORDING_STARTING)
        return 'Starting Recording';
      if (recordingState === Constants.recordingEvents.RECORDING_STOPPED) return 'Start Recording';
      if (recordingState === Constants.recordingEvents.RECORDING_STOPPING)
  ...
  const ScreenShareBTN = ({ isMobile, isTab }: ScreenShareBTNProps) => {
    ...
    return isMobile || isTab ? (
      <MobileIconButton
        id="screen-share-btn"
        tooltipTitle={
          presenterId ? (localScreenShareOn ? 'Stop Presenting' : undefined) : 'Present Screen'
        }
        buttonText={
          presenterId ? (localScreenShareOn ? 'Stop Presenting' : undefined) : 'Present Screen'
        }
  ...
  // Chat Button
  const ChatBTN = ({ isMobile, isTab }: ChatBTNProps) => {
  ...
  // Participants Button
  const ParticipantsBTN = ({ isMobile, isTab }: ParticipantsBTNProps) => {
  ...
  const MeetingIdCopyBTN = () => {
    ...
    return (
      ...
      >
        ...
        >
          <Typography sx={{ color: 'white', fontSize: '1rem' }}>{meetingId}</Typography>
          <IconButton
            sx={{ ml: 1 }}
            onClick={() => {
                ...
                {otherFeatures.map(({ icon }, index) => (
                  <Box
                    ...
                    sx={{
                      ...
                      justifyContent: 'center',
                      gridColumn:
                        icon === BottomBarButtonTypes.MEETING_ID_COPY ? 'span 7' : 'span 4',
                    }}
                  >
                    {icon === BottomBarButtonTypes.RAISE_HAND && (
